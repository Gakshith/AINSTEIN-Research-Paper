...
from src.AINSTEIN.entity.config_entity import InternalCritiqueConfig


class InternalCritiqueModel(BaseModel):
...
class InternalCritique:
    def __init__(self, config: InternalCritiqueConfig):
        self.config = config

    def get_internal_critique_score(self) -> float:
        parser = PydanticOutputParser(pydantic_object=InternalCritiqueModel)

        base_llm = HuggingFaceEndpoint(
            repo_id=self.config.internal_critique_model,
        ...
        average_internal_critique_score = (
            parsed.novelty + parsed.technical_feasibility + parsed.completeness
        ) / 3
        status = average_internal_critique_score >= self.config.internal_critique_threshold
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            f.write(
                f"Status: {status}\n"
                f"Average Score: {average_internal_critique_score:.2f}\n"
        ...
        return average_internal_critique_score
